  ...
  <v-container>
    <v-container class="hero">
      <v-row>
        <v-col class="d-flex flex-column justify-center" col="10">
          <h2 class="text-h2 font-weight-medium" :style="{ color: textColor }">
    ...
    <v-container fluid>
      ...
      <v-row dense justify="center" justify-md="space-between">
        <v-card class="mt-1 ml-1" v-for="item in featured" :key="item.name" :max-width="featuredSize"
          :max-height="featuredSize">
          <v-img :src="require(`@/assets/${item.img}`)" class="white--text align-end" :height="featuredSize / 2"
    ...
    <v-container>
      <PromoSection name="Proteins" description="Lorem ipsum" goTo="/shop?category=protein">
        <ShopItem v-for="item in proteins" v-bind:key="item.name" :item="item" class="ml-2 mt-2" />
      </PromoSection>
      <PromoSection name="BCAA" description="Lorem ipsum" goTo="/shop?brand=whey">
        <ShopItem v-for="item in proteins" v-bind:key="item.name" :item="item" class="ml-2 mt-2" />
      </PromoSection>
    </v-container>
  </v-container>
